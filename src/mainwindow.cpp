...
bool MainWindow::eventFilter(QObject *obj, QEvent *event)
{
  ...
  else if (event->type() == QEvent::MouseButtonRelease)
  {
    ...
    QMouseEvent *mouseEvent = static_cast<QMouseEvent *>(event);
    if (mouseEvent->button() == Qt::LeftButton)
      _bLeftMouseButtonPressed = false;
  ...
  else if (_bLeftMouseButtonPressed == true && event->type() == QEvent::MouseMove)
  {
    QMouseEvent *mouseEvent = static_cast<QMouseEvent *>(event);

    const QPoint delta = mouseEvent->globalPosition().toPoint() - _dragMousePos;
    move(_dragMainWindowPos.x() + delta.x(), _dragMainWindowPos.y() + delta.y());
  }
